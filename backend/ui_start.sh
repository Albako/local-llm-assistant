...
# Handle Ollama if needed (though it should be in a separate container)
if [[ "${USE_OLLAMA_DOCKER,,}" == "true" ]]; then
    echo "USE_OLLAMA is set to true, starting ollama serve."
    ollama serve &
fi

# Handle CUDA configuration
if [[ "${USE_CUDA_DOCKER,,}" == "true" ]]; then
    echo "CUDA is enabled, appending LD_LIBRARY_PATH to include torch/cudnn & cublas libraries."
    export LD_LIBRARY_PATH="$LD_LIBRARY_PATH:/usr/local/lib/python3.11/site-packages/torch/lib:/usr/local/lib/python3.11/site-packages/nvidia/cudnn/lib"
fi

# Handle Playwright if needed
if [[ "${WEB_LOADER_ENGINE,,}" == "playwright" ]]; then
    ...
    python -c "import nltk; nltk.download('punkt_tab')" 2>/dev/null || true
fi

# Handle HuggingFace Space configuration
if [ -n "$SPACE_ID" ]; then
